...
export class UserAuthComponent {
  ...
  registerForm = new FormGroup(
    ...
    {
      validators: [matchingPasswordsValidator(this.userService)],
      asyncValidators: [availableUsernameAndEmailValidator(this.userService)],
    }
  );

  changeAction(isInLogiView: boolean): void {
    const isClickingOnTheSameAction =
  ...
  setupHover(
    ...
    inactiveElement: HTMLButtonElement
  ): void {
    if (
      (hoveredElement.textContent?.includes('Sign in') &&
        !this.isInLoginView) ||
      (hoveredElement.textContent?.includes('Sign up') && this.isInLoginView)
    ) {
  ...
  setupUnhover(
    unhoveredElement: HTMLButtonElement,
    inactiveElement: HTMLButtonElement
  ): void {
    if (
      (unhoveredElement.textContent?.includes('Sign in') &&
        !this.isInLoginView) ||
      (unhoveredElement.textContent?.includes('Sign up') && this.isInLoginView)
    ) {
